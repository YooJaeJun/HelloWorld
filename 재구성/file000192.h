...
int main()
{
	pWORM wormHeadNode = (pWORM)malloc(sizeof(WORM));//이중연결리스트 헤드노드
	pWORM wormTailNode = (pWORM)malloc(sizeof(WORM));//이중연결리스트 테일노드
	pWORM addWorm = (pWORM)malloc(sizeof(WORM));//첫번째 웜몸통
	pITEM itemNode = (pWORM)malloc(sizeof(ITEM));//아이템용 단일 연결리스트
