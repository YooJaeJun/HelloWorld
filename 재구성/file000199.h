	...
	auto bfs = [&]()
	{
		...
		while (dq.empty() == false)
		{
			...
			forn(i, 4)
			{
				...
				if (dist[nx][ny][newBreaking] != maxn)
				{
					if (dist[nx][ny][newBreaking] < dist[x][y][newBreaking] + 1) continue;
				}

				dist[nx][ny][newBreaking] = dist[x][y][newBreaking] + 1;
				dq.push_back({ nx, ny, newBreaking });
